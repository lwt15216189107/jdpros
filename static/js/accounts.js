let goods = document.querySelector('.account .store .main-store ul')
let m = document.querySelector('.account .money li p span')
let sum = document.querySelector('.account .money .add-sum p span')
let price = document.querySelector('.account .foot .price span')
...
axios({
    method: 'get',
    url: `http://127.0.0.1:3000/getbuy`
}).then(data => {
    let sums = 0;
    if (data.data.length != 0) {
        for (let k of data.data) {
            ...
            `
            let money = k.price * k.num2
            sums += money
            m.innerHTML = `${sums}.`
            sum.innerHTML = `${sums}.`
            price.innerHTML = `${sums}.`
        }
    }
})

//结算页面点击去订单页面
let inline = document.querySelector('.foot .inline')
let mask = document.querySelector('.mask')
let yes = document.querySelector('.mask .btn .yes')
let no = document.querySelector('.mask .btn .no')
...
yes.onclick = function () {
    axios({
        method: 'get',
        url: `http://127.0.0.1:3000/state/1`
    }).then(data => {
...
no.onclick = function () {
    axios({
        method: 'get',
        url: `http://127.0.0.1:3000/state/2`
    }).then(data => {
        window.location.href = 'http://127.0.0.1:3000/html/order.html'
        mask.style.display = 'none'
    })
...
var provinceList = [
    {
        name: '北京市', cityList: [
            { name: '北京市', areaList: ["东城区", "西城区", "朝阳区", "丰台区", "石景山区", "海淀区", "门头沟区", "房山区", "通州区", "顺义区", "昌平区", "大兴区", "怀柔区", "平谷区", "密云区", "延庆区", "其他区"] },
        ]
    },
    {
        name: '天津市', cityList: [
            { name: '天津市', areaList: ["和平区", "河东区", "河西区", "南开区", "河北区", "红桥区", "东丽区", "西青区", "津南区", "北辰区", "武清区", "宝坻区", "滨海新区", "宁河区", "静海区", "蓟州区", "其他区"] },
        ]
    },
    {
        name: '上海市', cityList: [
            { name: '上海市', areaList: ["黄浦区", "徐汇区", "长宁区", "静安区", "普陀区", "虹口区", "杨浦区", "闵行区", "宝山区", "嘉定区", "浦东新区", "金山区", "松江区", "青浦区", "奉贤区", "崇明区", "其他区"] },
        ]
    },
    {
        name: '重庆市', cityList: [
            { name: '重庆市', areaList: ["万州区", "涪陵区", "渝中区", "大渡口区", "江北区", "沙坪坝区", "九龙坡区", "南岸区", "北碚区", "綦江区", "大足区", "渝北区", "巴南区", "黔江区", "长寿区", "江津区", "合川区", "永川区", "南川区", "璧山区", "铜梁区", "潼南区", "荣昌区", "开州区", "梁平区", "武隆区", "其他区"] },
...
provinceList.forEach(function (i, index) {
    var option = new Option(i.name, index)
    areas.add(option);
})
provinceList[0].cityList.forEach(function (j, index) {
    var option = new Option(j.name, index)
    citys.add(option);
})
provinceList[0].cityList[0].areaList.forEach(function (j, index) {
    var option = new Option(j, index)
    qus.add(option);
})

var area_index = areas.selectedIndex;
var citys_index = citys.selectedIndex;
//省发生改变
areas.onchange = function () {
    area_index = areas.selectedIndex;
    citys.innerHTML = '';
    provinceList[area_index].cityList.forEach(function (j, index) {
        var option = new Option(j.name, index)
        citys.add(option);

        qus.innerHTML = '';
        j.areaList.forEach(function (k, index) {
            var option = new Option(k, index)
            qus.add(option);
        })
    })
    // 市发生改变
    citys.onchange = function () {
        citys_index = citys.selectedIndex;
        qus.innerHTML = ''
        provinceList[area_index].cityList[citys_index].areaList.forEach(function (i, index) {
            var option = new Option(i, index)
            qus.add(option);
        })
    }
...
var address = document.querySelector('.account .address .i')
var ret = document.querySelector('.accounts .top i')
let masks = document.querySelector('.masks')
address.onclick = function () {
    masks.style.display = 'block'
}
let update = document.querySelector('.masks >span')
// let textarea = document.querySelector('.masks textarea')
// console.log(textarea);
let box = document.querySelector('.address .box')
console.log(box);
update.onclick = function () {
    let qu_index = qus.selectedIndex;
    let area = areas.children[area_index].innerHTML
    let city = citys.children[citys_index].innerHTML
    let qu = qus.children[qu_index].innerHTML
    let detail = document.querySelector('.masks textarea').value
    masks.style.display = 'none'
    box.innerHTML = ` <p class="province">
    <span>默认</span><span>${area}</span><span>${city}</span><span>${qu}</span>
</p>
<h2>${detail}</h2>
<p class="user-name">
    <span>刘文涛</span>
